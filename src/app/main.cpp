...
// 将srcFilePath指定的文件或者目录拷贝到tgtFilePath
...
static inline QStringList getPluginPaths()
{
    ...
    // patch versions
    // Qt Creator X.Y.Z可以从X.Y.（Z-1）等加载插件，因此添加当前和以前的补丁版本
...
{
    // 如果installSettingspath不为空且不是目录名，则报错并清空installSettingspath
    ...
    // 如果包含键为Settings/InstallSettings的设置项
...
int main(int argc, char **argv)
{
    ...
    // 手动确定各种命令行选项。我们不能使用插件管理器的常规方式，因为设置可以改变插件管理器的行为方式
    Options options = parseCommandLine(argc, argv);
    // 设置应用程序目录的绝对路径
    applicationDirPath(argv[0]);

    // 如果设置了QTC_DO_NOT_PROPAGATE_LD_PRELOAD环境变量则取消
...
#ifdef Q_OS_WIN
    // 如果为Windows系统并且未设置QT_OPENGL环境变量
